...
rw::V3d ClumpPos = {0.0f, 0.0f, 75.0f};
...
CreateTestScene(rw::World *world)
{
	...
	// Bottom panel
	clumpFrame = Scene.clump->getFrame();
	...
	world->addClump(Scene.clump);

	// Top panel
	clone = Scene.clump->clone();
...
//	ImGui::ShowDemoWindow(&show_demo_window);
	...
	rw::Frame *camFrame = Scene.camera->getFrame();
	camFrame->translate(&CameraPos, rw::COMBINEREPLACE);

	// ImGui::SliderFloat("Clump X", &ClumpPos.x, -100.0f, 100.0f);
	// ImGui::SliderFloat("Clump Y", &ClumpPos.y, -100.0f, 100.0f);
	// ImGui::SliderFloat("Clump Z", &ClumpPos.z, -100.0f, 100.0f);
	// rw::Frame *clumpFrame = Scene.clump->getFrame();
	// clumpFrame->translate(&ClumpPos, rw::COMBINEREPLACE);
