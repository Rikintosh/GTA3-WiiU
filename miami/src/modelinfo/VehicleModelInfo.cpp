...
ChooseComponent(int32 rule, int32 comps)
{
	...
	// identical cases....
	case 1:
		n = CGeneral::GetRandomNumberInRange(0l, CountCompsInRule(comps));
	...
	case 2:
		...
		return COMPRULE_COMPN(comps, n);
	case 3:
		n = CGeneral::GetRandomNumberInRange(0l, 1+CountCompsInRule(comps));
		if(n != 0)
...
CVehicleModelInfo::ChooseComponent(void)
{
	...
	if(ms_compsToUse[0] == -2){
		if(COMPRULE_RULE(m_compRules) && IsValidCompRule(COMPRULE_RULE(m_compRules)))
			...
			if(n)
				comp = comps[(int)CGeneral::GetRandomNumberInRange(0l, n)];
		}
	}else{
		...
		if(m_numColours > 1)
			for(i = 0; i < 8; i++){
				if(*col1 != m_lastColour1 || *col2 != m_lastColour2)
					break;
				n = CGeneral::GetRandomNumberInRange(0l, (int32)m_numColours);
				*col1 = m_colours1[n];
				*col2 = m_colours2[n];
			}
		m_lastColour1 = *col1;
