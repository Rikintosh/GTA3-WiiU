		...
		for ( ; particle != nil; _Next(particle, prevParticle, psystem, bRemoveParticle) )
		{
				...
				|| psystem->m_Type == PARTICLE_RAINDROP_2D )
			{
					...
					&& !CGame::IsInInterior() )
				{
					...
					if ( TheCamera.m_CameraAverageSpeed < 0.35f )
						vecWaterdropPos.x = (float)CGeneral::GetRandomNumberInRange(50l, int32(SCREEN_WIDTH) - 50);
					else
						vecWaterdropPos.x = (float)CGeneral::GetRandomNumberInRange(200l, int32(SCREEN_WIDTH) - 200);
					...
					{
						if ( TheCamera.m_CameraAverageSpeed < 0.35f )
							vecWaterdropPos.y  = (float)CGeneral::GetRandomNumberInRange(0l, int32(SCREEN_HEIGHT));
						else
							vecWaterdropPos.y  = (float)CGeneral::GetRandomNumberInRange(150l, int32(SCREEN_HEIGHT) - 200);
					}
