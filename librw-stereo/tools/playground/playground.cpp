...
//	postfxtest();
	...
	const char *filename = "stallion.dff";
	if(sk::args.argc > 1)
	...
	camera->m_aspectRatio = 400.0f/240.0f;
//	camera->m_near = 0.5f;
	camera->m_near = 1.5f;
//	camera->m_far = 450.0f;
	camera->m_far = 15.0f;
...
im2dtest(void)
{
	...
	static struct
	{
		...
		{ 400.0f,   0.0f,   0, 255, 0, 128,    1.0f, 0.0f },
		{   0.0f, 240.0f,   0, 0, 255, 128,    0.0f, 1.0f },
		{ 400.0f, 240.0f,   0, 255, 255, 128,  1.0f, 1.0f },
	};
	Im2DVertex verts[4];
	...
	for(i = 0; i < 4; i++){
		...
		verts[i].setU(vs[i].u + 0.5f/400.0f, recipZ);
		verts[i].setV(vs[i].v + 0.5f/240.0f, recipZ);
	}
...
im2dtest2(void)
{
	...
	for(i = 0; i < 4; i++){
		...
		verts[i].setScreenX((vs[i].x*recipZ + 0.5f) * 400.0f);
		verts[i].setScreenY((vs[i].y*recipZ + 0.5f) * 240.0f);
...
im3dtest(void)
{
	...
	static struct
	{
		...
		{ -1.0f, -1.0f, -1.0f,   255, 0, 0, 255,    0.0f, 0.0f },
		{ -1.0f,  1.0f, -1.0f,   0, 255, 0, 255,    0.0f, 1.0f },
		{  1.0f, -1.0f, -1.0f,   0, 0, 255, 255,    1.0f, 0.0f },
		{  1.0f,  1.0f, -1.0f,   255, 0, 255, 255,  1.0f, 1.0f },
...
Draw(float timeDelta)
{
	...
	static rw::RGBA clearcol = { 20, 10, 40, 0xFF };
