...
// 	int32   format = ras->format | Raster::MIPMAP | Raster::TEXTURE | Raster::DONTALLOCATE;
// 	Raster *newras = Raster::create(img->width, img->height, img->depth, format, PLATFORM_3DS);

// 	rw::c3d::rasterFromImage_etc1(newras, img);
// 	img->destroy();
...
Raster::convertTexToCurrentPlatform(rw::Raster *ras)
{
	...
	// else if(rw::platform == PLATFORM_3DS){
	// 	Raster *newras = ras_to_etc1(ras);
	// 	if(newras){
	// 		ras->destroy();
	// 		return newras;
	// 	}
...
#undef min	
	for(int i = 1; i < numLevels; i++){
		...
		newras->unlock(i);
		ras->unlock(i);
		img->destroy(); /* memory was leak :DDD */
	}
	ras->destroy();
	ras = newras;
	return ras;
